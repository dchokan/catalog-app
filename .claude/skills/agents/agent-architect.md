...
**Where the file lives.** This repo keeps agent sources in `agents/` at the root (alongside this file). But Claude Code only _auto-discovers_ agents from `.claude/agents/` (project) and `~/.claude/agents/` (user). For a generated agent to be live in a session, it must be placed/copied there. Always state this when you deliver.
